...
const IASupportPage: React.FC = () => {
  ...
  const loadConversas = async () => {
    // For now, use localStorage to simulate conversation history
    try {
      const savedConversas = localStorage.getItem('ia_conversas');
      if (savedConversas) {
  ...
  const enviarPergunta = async () => {
    ...
    try {
      ...
      const novasConversas = [...conversas, novaConversa];
      setConversas(novasConversas);
      
      // Save to localStorage temporarily
      localStorage.setItem('ia_conversas', JSON.stringify(novasConversas));
  ...
  const simularRespostaIA = async (pergunta: string): Promise<string> => {
    ...
    const respostasExemplo = [
      "Com base na sua pergunta sobre gestão de oficina, recomendo implementar um sistema de controle de estoque mais rigoroso para evitar faltas de peças importantes.",
      "Para melhorar o atendimento ao cliente, sugiro criar um sistema de agendamento online e acompanhamento de serviços em tempo real.",
      "Sobre precificação de serviços, é importante considerar custos de mão de obra, peças, margem de lucro adequada e pesquisa de mercado local.",
      "Para organizar melhor sua oficina, recomendo implementar um sistema de gestão integrado que controle desde o orçamento até a entrega do veículo."
    ];

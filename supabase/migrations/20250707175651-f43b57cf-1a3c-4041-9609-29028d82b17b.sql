-- Primeiro, vamos garantir que a tabela oficinas seja a principal para oficinas cadastradas
-- E que haja relacionamento correto com clients

-- Garantir que existe o campo oficina_id na tabela clients (já deve existir)
...
CREATE POLICY IF NOT EXISTS "Admins can view all oficinas" 
  ON public.oficinas 
  FOR SELECT 
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Admins can update all oficinas" 
  ON public.oficinas 
  FOR UPDATE 
  USING (
    EXISTS (
...
-- Atualizar políticas na tabela clients para usar oficina_id corretamente
-- Política para garantir que clientes sejam vinculados à oficina correta
CREATE POLICY IF NOT EXISTS "Clients must have correct oficina_id" 
  ON public.clients 
  FOR INSERT 
  WITH CHECK (
